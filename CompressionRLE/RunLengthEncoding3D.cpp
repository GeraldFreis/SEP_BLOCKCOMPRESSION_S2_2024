...
void Compression::runLengthEncoding(){
  int x_count = headers[0], y_count = headers[1], z_count = headers[2];
  int parent_x = headers[3], parent_y = headers[4], parent_z = headers[5];

  for (int z = 0; z < z_count; z++) {
    for (int y = 0; y < y_count; y++) {
      for (int x = 0; x < x_count; x++) {
        if (visited[z][y][x]) continue;  // Skip already visited blocks
        ...
        int parent_x_end = std::min(parent_x_start + parent_x, x_count);
        int parent_y_end = std::min(parent_y_start + parent_y, y_count);
        int parent_z_end = std::min(parent_z_start + parent_z, z_count);

        // Calculate max_x_size
        int max_x_size = 1;
        while (x + max_x_size < parent_x_end &&
               blocks[z][y][x + max_x_size] == label &&
               !visited[z][y][x + max_x_size]) {
          max_x_size++;
        }

        // Calculate max_y_size
        int max_y_size = 1;
        while (y + max_y_size < parent_y_end) {
          bool uniform_y = true;
          for (int dx = 0; dx < max_x_size; dx++) {
            if (blocks[z][y + max_y_size][x + dx] != label || visited[z][y + max_y_size][x + dx]) {
              uniform_y = false;
              break;
            }
          }
          if (uniform_y) max_y_size++;
          else break;
        }

        // Calculate max_z_size
        int max_z_size = 1;
        while (z + max_z_size < parent_z_end) {
          bool uniform_z = true;
          for (int dy = 0; dy < max_y_size; dy++) {
            for (int dx = 0; dx < max_x_size; dx++) {
              if (blocks[z + max_z_size][y + dy][x + dx] != label || visited[z + max_z_size][y + dy][x + dx]) {
                uniform_z = false;
                break;
              }
            }
          ...
          if (uniform_z) max_z_size++;
          else break;
        }

        // Mark visited blocks
        for (int dz = 0; dz < max_z_size; dz++) {
          for (int dy = 0; dy < max_y_size; dy++) {
            for (int dx = 0; dx < max_x_size; dx++) {
        ...
        x += max_x_size - 1;  // Move x to the end of the compressed block
      }
    }
  }
