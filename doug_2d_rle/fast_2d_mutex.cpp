...
enum LayerState { used = 0, full = 1, taken = 2 };
...
std::mutex using_buffers;
volatile Layer layers[NUM_LAYERS];
...
void compression_thread() {
    ...
    for (;;) {
        ...
        for (int i=0; i<NUM_LAYERS; i++) {
            if (layers[i].state != LayerState::full) continue;
            using_buffers.lock();

            // check if its still avaliable
            if (layers[i].state != LayerState::full) {
                using_buffers.unlock();
                continue;
            }

            layers[i].state = LayerState::taken;
            layer_index = i;
            using_buffers.unlock();
            break;
        }
        ...
        input_buffer = layers[layer_index].data;
        z = layers[layer_index].z;
        ...
        layers[layer_index].state = LayerState::used;
    }
    serialiser.flush();
}
...
int main (int argc, char *argv[]) {
    ...
    for (int i=0; i<NUM_LAYERS; i++) {
        ...
        layers[i].data = buf;
        layers[i].z = z++;
        layers[i].state = LayerState::full;
    }

    // std::thread t2(compression_thread);
    // std::thread t3(compression_thread);

    for (;z < block_sizes.count_z;) {
        for (int i=0; i<NUM_LAYERS; i++) {
            if (layers[i].state != LayerState::used) continue;
            std::cin.read(layers[i].data, INPUT_PLANE_SIZE);
            layers[i].z = z++;
            layers[i].state = LayerState::full;
            break;
        }
